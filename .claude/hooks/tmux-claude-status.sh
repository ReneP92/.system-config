#!/usr/bin/env bash
# Marks the tmux pane running this Claude session as busy/waiting/idle.
# Usage: tmux-claude-status.sh <busy|waiting|clear>
[ -n "$TMUX" ] && [ -n "$TMUX_PANE" ] || exit 0
case "$1" in
  clear) tmux set-option -p -t "$TMUX_PANE" -u @claude_state ;;
  *)
    tmux set-option -p -t "$TMUX_PANE" @claude_state "$1"
